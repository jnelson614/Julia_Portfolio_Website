...
var startKeys=[83, 32, 40, 13];//An array of keys to press to start the game, read the instructions in the index to determine what keycodes should go here
var eggsMissed=0;
var score=0;
...
function increaseScore(){
	...
	$("#score").children("span").text(score+=.5);
	if(score>5){
		dropAnEgg("medium");
	}
	if(score>10){
...
$(document).ready(function(){
	$(window).on("keydown",function(e){
		...
		if(e.which==32 && $(".egg").length>0){ //space bar
		...
		if(leftKeys.indexOf(e.which)!=-1){
			...
			return moveBasketLeft();
		}
		else if(rightKeys.indexOf(e.which)!=-1){
			//What function should be called here?
			return moveBasketRight();
		}
	...
	$("#egg-drop-game").on("animationEnd webkitAnimationEnd",".egg",function(){
		...
		console.log(eggMargin);
		console.log(basketRightEdge);
		console.log(basketMargin);
		...
		eggsMissed++;
		if(eggsMissed==2){//THEN game is over
